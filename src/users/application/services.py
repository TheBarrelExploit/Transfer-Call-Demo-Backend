...
class UserService(UserServiceUser):
    ...
    async def update_user(self, id: str, data_user: Dict[str, Any])-> Optional[UserBase]:
        user = await self.repository.find_by_id(id)
        if not user:
            raise UserNotFoundException(f"User with id {id} not found")
    ...
    async def change_password(self, id: str, old_password: str, new_password: str) -> UserBase:
        user = await self.repository.find_by_id(id)
        if not user:
            raise UserNotFoundException(f"User with id {id} not found")
